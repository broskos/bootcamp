...
```
![NOTE] Ensure to restart NetworkManager or dnsmasq etc. after making DNS record changes
...
[!NOTE] At different attemtpts, it took from 4 to 10 minutes for the CSR to appear *after* the node had reached `provisioned` state
...
[!NOTE] Automatic draining can be disabled by annotating `machine.openshift.io/exclude-node-draining` in a specific machine
...
![NOTE] You can use `oc get clusterversions.config.openshift.io` to find the cluster's version and define the OCP_VERSION variable accordingly
...
![NOTE]Update the ssh key and other values according to your environment
